...
def test_LoginUser_GivenAnIncorrectPassword_ShouldReturnFalse():
    #Prepare
    try :
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
        ...
        if(db != None):
            ...
            #Act
            result = db.login(email, password+"incorrect")
...
def test_LoginUser_GivenAUserThatDoesntExist_ShouldReturnFalse():
    #Prepare
    try :
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
        ...
        if(db != None):
            ...
            #Act
            result = db.login(email+"@doesnotexisit", password)
